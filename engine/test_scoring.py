...
def test_order_below_one_crore_does_not_fire():
    s = results(document_type="ORDER_WIN",
                orders=[OrderWin(unit="crore", raw_value=0.5, customer="NTPC")])
    assert "ORDER_WIN" not in scoring.score_filing(s)["rules_hit"]


def test_order_of_one_crore_fires():
    s = results(document_type="ORDER_WIN",
                orders=[OrderWin(unit="crore", raw_value=1.0, customer="NTPC")])
...
def test_order_scoring_is_logarithmic():
    small = scoring.score_filing(results(orders=[OrderWin(unit="crore", raw_value=5.0)]))["score"]
    mid = scoring.score_filing(results(orders=[OrderWin(unit="crore", raw_value=250.0)]))["score"]
    big = scoring.score_filing(results(orders=[OrderWin(unit="crore", raw_value=5000.0)]))["score"]
    assert small < mid < big
    assert abs(big - config.ORDER_WEIGHT) < 0.01  # past ORDER_FULL_CR, maxed


def test_multiple_orders_are_summed():
    s = results(orders=[OrderWin(unit="crore", raw_value=100.0), OrderWin(unit="crore", raw_value=150.0)])
    r = scoring.score_filing(s)
...
def test_order_with_undisclosed_value_is_noted_but_not_scored():
    s = results(orders=[OrderWin(raw_value=None, unit="", customer="Indian Railways")])
    r = scoring.score_filing(s)
...
def test_all_three_rules_max_at_one_hundred():
    s = results(
        ...
        orders=[OrderWin(unit="crore", raw_value=5000.0)],
        document_type="BOTH",
    )
    r = scoring.score_filing(s)
